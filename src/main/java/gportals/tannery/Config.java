...
import static gportals.tannery.Tannery.tannery;

public class Config {

  // Recipes config file
  static File recipesFile = new File(tannery.getDataFolder(), "recipes.yml");
  static FileConfiguration recipesConf = YamlConfiguration.loadConfiguration(recipesFile);

  // Stations config file
  static File stationsFile = new File(tannery.getDataFolder(), "stations");
  ...
  public static ItemStack enchantArmor(ItemStack item, Hide recipe) {
    ...
    tannery.getLogger().info(String.valueOf(recipesConf.getKeys(true)));

    ItemMeta meta = item.getItemMeta();

    switch (item.getType()) {
      case LEATHER_HELMET:
        ConfigurationSection confHelmet =
                conf.getConfigurationSection("crafting.helmet.enchantments");
        for (String key : confHelmet.getKeys(false)) {
          meta.addEnchant(
                  Enchantment.getByKey(NamespacedKey.minecraft(key)), confHelmet.getInt(key), true);
        }
        break;
      case LEATHER_CHESTPLATE:
        ConfigurationSection confChest =
                conf.getConfigurationSection("crafting.chestplate.enchantments");
        for (String key : confChest.getKeys(false)) {
          meta.addEnchant(
                  Enchantment.getByKey(NamespacedKey.minecraft(key)), confChest.getInt(key), true);
        }
        break;
      case LEATHER_LEGGINGS:
        ConfigurationSection confLeg =
                conf.getConfigurationSection("crafting.leggings.enchantments");
        for (String key : confLeg.getKeys(false)) {
          meta.addEnchant(
                  Enchantment.getByKey(NamespacedKey.minecraft(key)), confLeg.getInt(key), true);
        }
        break;
      case LEATHER_BOOTS:
        ConfigurationSection confBoots =
                conf.getConfigurationSection("crafting.boots.enchantments");
        for (String key : confBoots.getKeys(false)) {
          meta.addEnchant(
                  Enchantment.getByKey(NamespacedKey.minecraft(key)), confBoots.getInt(key), true);
        }
        break;
    }
  ...
  public static ArrayList<Station> getStations() {
    ...
    for (String key : stationsConf.getKeys(false)) {
      ...
      Station station =
              new Station(
                      (Location) stationsConf.get(key + ".location"),
  ...
  public static Station getStation(Block block) {
    Station get = null;
    for (Station station : getStations()) {
      if (station.getLocation().equals(block.getLocation())) get = station;
    }
